    ...
    try {
      ...
      dispatch({
        ...
        payload: {
          ...
          message: "Your requested report for regrading should appear momentarily",
          success: true,
        },
      });
    } catch (error: any) {
          ...
          <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            ...
            <div className="mt-2">
              <p className="text-sm leading-5 text-gray-500">
                To trigger regrading for currently assigned students for this updated config press Regrade, otherwise
                Ignore to continue.
              </p>
            </div>
          </div>
  ...
  async function updateYaml() {
    try {
      ...
      if (!configError) {
        ...
        dispatch({
          type: "configYAMLUpdate",
          payload: { assignmentConfigId: parseInt(assignmentConfigId as string, 10) },
        });
      } else {
        dispatch({
          type: "showNotification",
          payload: {
            success: false,
            title: "Config Error",
            message: JSON.parse(configError).configFormatError.throwableSummary.message,
          },
        });
      }
    } catch (error: any) {
