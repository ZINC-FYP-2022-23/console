...
function Courses() {
  ...
  if (loading) {
    return <tbody className="bg-white divide-y divide-cool-gray-200"></tbody>;
  }
  return (
    <>
            ...
            <tr className="bg-white hover:bg-gray-50" key={course.id}>
              <td className="px-6 py-4 whitespace-no-wrap">
                <div className="flex items-center">
                  <div className="ml-4">
                    <div className="text-sm leading-5 font-medium text-gray-900">{course.code}</div>
                    <div className="text-sm leading-5 text-gray-500">{course.name}</div>
                  </div>
                </div>
              </td>
                ...
                <div className="flex items-center space-x-2">
                  <div className="flex flex-shrink-0 -space-x-1">
                    {course.teachingStaff.map((staff) => (
                      <div
                        key={staff.user.name}
                        className="max-w-none h-6 w-6 rounded-full bg-gray-600 text-xs text-white shadow-solid flex justify-center items-center"
                      >
                        {staff.user.initials}
                      </div>
                    ))}
          ...
          <div className="min-w-0 flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <div className="text-sm leading-5 font-medium text-cse-600 truncate">
                {role}
                <span className="ml-1 font-normal text-gray-500">in {course.code}</span>
              </div>
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-md">
          <ul>
            {courses.map(({ role, course, id, permission }) => (
              <UserCoursePermissionCollapsible course={course} permission={permission} role={role} key={id} />
            ))}
          </ul>
        </div>
      </div>
