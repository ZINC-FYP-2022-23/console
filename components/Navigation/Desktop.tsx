...
function AdminNavigation() {
  return (
    <>
      <h2 className="mt-6 text-xs font-semibold text-gray-600 uppercase tracking-wider">Administration</h2>
                ...
                {!loading &&
                  data.user.courses.map(({ course }) => (
                    <a
                      key={course.code}
                      href="#"
                      className="group select-none ml-1 w-full flex items-center pl-11 pr-2 py-2 text-sm leading-5 font-medium text-gray-600 rounded-md hover:text-gray-900 hover:bg-gray-50 focus:outline-none focus:text-gray-900 focus:bg-gray-50 transition ease-in-out duration-150"
                    >
                      {course.code}
                    </a>
                  ))}
...
export function Toolbar() {
  return (
    <div className="hidden md:flex items-center">
      ...
      <button
        className="ml-4 p-2 border-2 h-8 w-8 flex items-center justify-center border-transparent rounded-full focus:bg-cse-600 focus:outline-none text-gray-400 hover:text-gray-200 transition ease-in-out duration-150"
