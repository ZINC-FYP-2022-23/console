    ...
    try {
      ...
      dispatch({
        ...
        payload: {
          ...
          message: "Changes to grading policy has been saved",
        },
      });
      onChange();
    } catch (error: any) {
      ...
      try {
        ...
        dispatch({
          ...
          payload: {
            ...
            message: "Grading package has been uploaded successfully",
          },
        });
      } catch (error: any) {
        dispatch({ type: "showNotification", payload: { success: false, title: "Error", message: error.message } });
        ...
        <ul className="px-4 py-5 sm:p-6">
          <li className="flex">
            ...
            <p className="text-sm">
              ...
              The files that you (the TAs) need in order for the grading to succeed. This may include parts of the
              skeleton, and additional files for e.g. custom test cases and testing frameworks.
            </p>
          </li>
          <li className="flex">
            <FontAwesomeIcon className="text-blue-400 mr-2 mt-2" icon={["fad", "exclamation-circle"]} />
          ...
          <nav className="-mb-px flex space-x-6">
            {["Provided", "Skeleton", "Template"].map((folder) => (
              <a
                ...
                className={`cursor-pointer whitespace-no-wrap pb-4 px-1 border-b-2 ${
                  selectedFolder === folder.toLowerCase()
                    ? "border-cse-500 font-medium text-sm leading-5 text-cse-600 focus:outline-none focus:text-cse-800 focus:border-cse-700"
                ...
                <li key={node.name} className="pl-3 pr-4 py-3 flex items-center justify-between text-sm leading-5">
                  <div className="w-0 flex-1 flex items-center">
                    <FontAwesomeIcon
                      ...
                      icon={["fad", node.type === "directory" ? "folder" : "file"]}
                    />
                    <span className="ml-2 flex-1 w-0 truncate">{node.name}</span>
                  </div>
                </li>
              ))}
            </ul>
          ...
          {fileTree === null && (
            <div className="bg-gray-200 rounded-md p-12 m-8 flex flex-col justify-center items-center">
              ...
              <p className="text-xs mt-1 text-gray-500 text-center">
                You can import the required grading package files via uploading from below
              </p>
            </div>
          )}
        </div>
                ...
                <div className="max-w-lg flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md outline-none">
                  <div className="text-center">
                    ...
                    <p className="mt-1 text-sm text-gray-600">
                      ...
                      <button
                        type="button"
                        className="mr-1 font-medium text-cse-600 hover:text-cse-500 focus:outline-none focus:underline transition duration-150 ease-in-out"
                      >
                        Upload a file
                      </button>
                      or drag and drop
                    </p>
                    <p className="mt-1 text-xs text-gray-500">zipped source files and folders up to 10MB</p>
                  </div>
                </div>
              </div>
            </div>
