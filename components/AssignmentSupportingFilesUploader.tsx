...
function FileWrap({ existing, file, children }) {
  ...
  if (!existing) {
    return (
      <div
        className="flex flex-row px-2 py-1 border border-gray-300 bg-gray-200 hover:shadow-md transition-all duration-350 ease-in-out justify-between rounded w-40 items-center gap-x-1 w-full"
        ref={drag}
      >
        {children}
      </div>
  ...
  return (
    <FileWrap existing={existing} file={file}>
      <div className="flex flex-row items-center space-x-2 truncate">
        ...
        <div className="text-gray-700 font-medium text-sm w-fit truncate ...">{path}</div>
      </div>
      <button
        onClick={() => {
...
function DockingFile({ files, file, setFiles }) {
  ...
  return (
    <div
      className="flex flex-row px-2 py-1 border border-gray-300 bg-gray-200 hover:shadow-md transition-all duration-350 ease-in-out justify-between rounded items-center gap-x-1 w-full"
      ref={drag}
    >
      <div className="flex flex-row items-center space-x-2 truncate">
        <div className="w-3 text-gray-600 mx-1">
        ...
        <div className="text-gray-700 font-medium text-sm w-fit truncate ...">{file.path}</div>
      </div>
    ...
    <>
      <div ref={reference} className="w-full p-4 rounded bg-gray-100">
        <div className="flex flex-col space-y-2 max-h-44 overflow-y-scroll" ref={drop}>
          {files.map((file) => (
            <DockingFile key={file} file={file} files={files} setFiles={setFiles} />
          ))}
        </div>
      </div>
      <div
...
function FileList({ existingFiles, name }) {
  ...
  return (
    <div className="rounded border bg-gray-100 p-2 shadow flex flex-col space-y-2 w-60" ref={drop}>
      ...
      <div className="text-sm text-gray-500 px-1 h-10">{description}</div>

      {[...existingFilesState, ...cachedFiles]
        .map((file) => {
          ...
          return {
            deleteFile: async (file) => {
              ...
              setExistingFilesState([
                ...new Set(existingFilesState.filter((fileState) => fileState.path !== file.path)),
              ]);
...
function AssignmentSupportingFilesUploader() {
  ...
  const handleSubmit = () => async () => {
    ...
    Object.entries(uploadFiles).forEach(([folder, files]) => {
            ...
            <Dropzone onDrop={addDockingFiles}>
              {({ getRootProps, getInputProps }) => (
                <div className="grid grid-cols-2 shadow-md p-2 rounded-md w-3/4">
                  <div {...getRootProps()} className="rounded p-10 flex flex-col items-center gap-y-4 cursor-pointer">
                    <input {...getInputProps()} />
                    <div className="text-gray-600 text-6xl">
                      <FontAwesomeIcon icon={["fad", "upload"]} />
                    </div>
                    <p className="text-center text-gray-600">Drag and Drop file or click to select files</p>
                  </div>
