...
function SelectMenu({ semesters = [] }: SemesterSelectMenuProps) {
  ...
  return (
    <div className="space-y-3">
      <label
        id="listbox-label"
        className="block text-xs leading-5 font-semibold text-gray-700 uppercase tracking-wider"
      >
        Semester
      </label>
      <div className="relative">
        <Menu>
          ...
          >
            <span className="block truncate">{semesters.find((semester) => semester.id === activeSemester)?.name}</span>
...
function MenuOption({ title, selected, href }: MenuOptionProps) {
  ...
  return (
    <div onMouseEnter={() => setHighlighted(true)} onMouseLeave={() => setHighlighted(false)}>
      <Menu.Item
        as="li"
        id="listbox-option-0"
        role="option"
      ...
      >
        <Link href={href.includes("//") ? href.replace("//", "/") : href}>
          <span
            ...
            } block truncate transition duration-150 ease-in-out`}
          >
            {title}
          </span>
        </Link>
        ...
        {selected && (
          <span
            className={`${
              highlighted ? "text-white" : "text-cse-600"
