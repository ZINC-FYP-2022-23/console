...
export function Assignments({ assignmentConfigs, sections }: AssignmentsProps) {
  return (
    <>
        ...
        <table className="min-w-full max-h-9/12 divide-y divide-cool-gray-200">
          ...
          <tbody className="bg-white divide-y divide-cool-gray-200">
            {assignmentConfigs.length > 0 &&
              assignmentConfigs.map((assignmentConfig) => (
...
export function AssignmentRow({ config, sections }) {
  ...
  const sectionsString = sections
    .filter(
      (section) =>
        config.affected_users
          .map(({ user_id }) => user_id)
          .filter((id) => section.users.map((user) => user.user_id).includes(id)).length > 0,
    )
